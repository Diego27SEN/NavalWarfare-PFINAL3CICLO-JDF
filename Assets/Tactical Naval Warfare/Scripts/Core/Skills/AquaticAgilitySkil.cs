...
public class AquaticAgilitySkil : MonoBehaviour
{
    ...
    private IEnumerator SpeedRoutine()
    {
        ...
        Debug.Log($"[Agilidad Acuatica] Velocidad original guardada: {originalSpeed}");

        // Aplicamos el buff
        ship.moveSpeed = originalSpeed + bonusVelocity;
        Debug.Log($"[Agilidad Acuatica] Activada: +{bonusVelocity}. Velocidad actual: {ship.moveSpeed}");

        yield return new WaitForSeconds(timeDuration);

        // Restauramos el valor exacto original
        ship.moveSpeed = originalSpeed;
        Debug.Log($"[Agilidad Acuatica] Terminada. Velocidad restaurada a: {ship.moveSpeed}");

        Destroy(this);
        Debug.Log("[Agilidad Acuatica] Componente destruido.");
    }
}
